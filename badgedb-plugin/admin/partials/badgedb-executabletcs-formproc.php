...
/**
 * Handles form submissions from the executable test case editor.
 *
...
//error_log("In etcs form proc");
 ...
 if ($isPost && $isAdmin) {
     ...
     if ($_POST['whichform'] == 'new') {
         include_once(plugin_dir_path(__FILE__) . 'badgedb-executabletcs-new-proc.php');
     }//end if it's a new catagory.

     if ($_POST['whichform'] == 'delete') {
        include_once(plugin_dir_path(__FILE__) . 'badgedb-executabletcs-delete-proc.php');
     }

     if ($_POST['whichform'] == 'update') {
      include_once(plugin_dir_path(__FILE__) . 'badgedb-executabletcs-edit-proc.php');
   }
